...
// Hint for forgot password form (public - no auth)
app.get("/api/admin/forgot-password-hint", async (req, res) => {
  const cfg   = await getSettings();
  const email = (cfg.admin_email || "").trim();
  const hint  = email.includes("@")
    ? email.replace(/^(.{2}).+(@.+)$/, "$1***$2")
    : "your configured admin email";
  res.json({ hint });
});

// ══ ADMIN FORGOT PASSWORD (no auth needed — called from login page) ══
// Step 1: Send OTP to admin_email (no token required)
  ...
  try {
    ...
    if (!adminEmail || !adminEmail.includes("@")) {
      return res.status(400).json({
        error: "Admin email not configured. Ask your system administrator to set it in API Keys & Email settings."
      });
    }

    const otp = generateOTP();
    saveOTP(adminEmail, otp, "admin_forgot_password");

    const html = `<!DOCTYPE html><html><body style="margin:0;padding:20px;background:#f5f7fa;font-family:Arial,sans-serif">
<div style="max-width:480px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
  <div style="background:linear-gradient(135deg,#1D4ED8,#60A5FA);padding:24px 28px;color:#fff">
    <div style="font-size:36px;margin-bottom:10px">🔐</div>
    <h2 style="margin:0 0 4px;font-size:20px">Admin Password Reset</h2>
    <p style="margin:0;opacity:.85;font-size:13px">MediCare+ Hospital Management System</p>
  </div>
  <div style="padding:28px;font-size:14px;color:#333;line-height:1.8">
    <p>A password reset was requested for the Admin Panel.</p>
    <div style="background:#f0f4ff;border:2px solid #3B82F6;border-radius:12px;padding:22px;text-align:center;margin:20px 0">
      <div style="font-size:12px;color:#666;margin-bottom:10px;font-weight:600;text-transform:uppercase;letter-spacing:1px">Your One-Time OTP</div>
      <div style="font-size:40px;font-weight:900;letter-spacing:10px;color:#1D4ED8;font-family:monospace">${otp}</div>
      <div style="font-size:12px;color:#999;margin-top:10px">Valid for 10 minutes only</div>
    </div>
    <p>Enter this OTP on the admin login page to set a new password.</p>
    <p style="color:#dc2626;font-size:13px"><strong>⚠️ Do not share this OTP with anyone.</strong></p>
  </div>
  <div style="padding:14px 28px;text-align:center;font-size:11px;color:#aaa;background:#f5f7fa">
    MediCare+ HMS · If you did not request this, ignore this email
  </div>
</div></body></html>`;

    const result = await sendEmail(adminEmail, "🔐 Admin Password Reset OTP — MediCare+", html, null, "admin_forgot_otp");

    if (result.skipped || result.error) {
      return res.status(503).json({
        error: result.error || "Email not configured. Set Brevo API key and sender email in API Keys settings first."
      });
    }
...
// Step 2: Verify OTP + set new admin password (no token required)
app.post("/api/admin/reset-admin-password", async (req, res) => {
  try {
    ...
    const check = verifyOTP(adminEmail, otp, "admin_forgot_password");
    if (!check.valid) return res.status(400).json({ error: check.error });
    ...
    console.log("✅ Admin password reset via forgot-password OTP flow");
    res.json({ message: "Password reset successfully! You can now login with your new password." });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ════════════════════════════════════════════════════════════════════
// SOCKET.IO  — Ambulance Tracking System
// ════════════════════════════════════════════════════════════════════
...
// ── In-memory live store (never persisted — always fresh) ──────────
const liveStore = {
  ambulances: {},   // { ambulanceId: { lat, lng, speed, ts, driverName, destination } }
  officers:   {},   // { officerId:   { lat, lng, ts, name, chowkId, chowkName } }
};
...
// ── Calculate ETA (seconds) from distance (m) and speed (km/h) ───
function calcETA(distanceM, speedKmh) {
  const spd = speedKmh > 2 ? speedKmh : 40; // default 40 km/h if GPS speed unreliable
  return Math.round((distanceM / (spd * 1000/3600)));
}

// ── Find nearest chowk from a list ───────────────────────────────
async function getNearestChowk(lat, lng) {
  const { data: chowks } = await supabase.from("chowks").select("*").eq("is_active", true);
  if (!chowks || !chowks.length) return null;
  let nearest = null, minDist = Infinity;
  for (const c of chowks) {
...
// ── Detect chowks along a route (within 300 m) ────────────────────
async function getChowksOnRoute(routeCoords) {
  const { data: chowks } = await supabase.from("chowks").select("*").eq("is_active", true);
  if (!chowks || !chowks.length || !routeCoords || !routeCoords.length) return [];
  const onRoute = [];
  for (const chowk of chowks) {
    let minDist = Infinity;
    for (const [lng, lat] of routeCoords) {
      const d = haversine(lat, lng, chowk.lat, chowk.lng);
      if (d < minDist) minDist = d;
    }
    if (minDist <= 300) onRoute.push({ ...chowk, distanceM: Math.round(minDist) });
  }
  return onRoute.sort((a, b) => a.distanceM - b.distanceM);
}

// ── Get nearest hospital ──────────────────────────────────────────
async function getNearestHospital(lat, lng) {
  const { data: hospitals } = await supabase.from("hospitals").select("*").eq("is_active", true);
  ...
  return { ...nearest, distanceM: Math.round(minDist) };
}

// ── Dedup alert tracker (prevent spam) ────────────────────────────
const alertSent = {}; // { `${ambulanceId}_${chowkId}_${level}` : timestamp }
function shouldSendAlert(ambulanceId, chowkId, level) {
  const key = `${ambulanceId}_${chowkId}_${level}`;
  const last = alertSent[key];
  if (last && Date.now() - last < 60000) return false; // cooldown 60s
  alertSent[key] = Date.now();
  return true;
}

// ════════════════════════════════════════════════════════════════════
// AMBULANCE TRACKING REST ENDPOINTS
// ════════════════════════════════════════════════════════════════════

// Driver updates location (called every 5–10s from driver app)
app.post("/api/ambulance/update-location", async (req, res) => {
  try {
    const { ambulanceId, driverName, lat, lng, speed, destination, routeCoords } = req.body;
    if (!ambulanceId || !lat || !lng) return res.status(400).json({ error: "ambulanceId, lat, lng required" });

    // Update in-memory store
    liveStore.ambulances[ambulanceId] = {
    ...
    // Broadcast to all admin/map watchers
    io.emit("ambulance:location", { ambulanceId, lat, lng, speed, driverName, destination, ts: Date.now() });

    // ── ETA-based alerts for chowks along route ──────────────────
    let alerts = [];
    if (routeCoords && routeCoords.length) {
      const onRouteChowks = await getChowksOnRoute(routeCoords);
      for (const chowk of onRouteChowks) {
        const distToChowk = haversine(lat, lng, chowk.lat, chowk.lng);
        const etaSec      = calcETA(distToChowk, parseFloat(speed) || 40);
        const etaMin      = etaSec / 60;

        let alertLevel = null;
        if      (etaMin <= 2)  alertLevel = "critical";
        else if (etaMin <= 5)  alertLevel = "urgent";
        else if (etaMin <= 10) alertLevel = "normal";

        if (alertLevel && shouldSendAlert(ambulanceId, chowk.id, alertLevel)) {
          const payload = {
            ambulanceId, driverName, lat, lng, speed,
            chowkId: chowk.id, chowkName: chowk.name,
            etaSec: Math.round(etaSec), etaMin: etaMin.toFixed(1),
            alertLevel, destination, ts: Date.now()
          };
          // Send to that chowk's room (all officers assigned there)
          io.to(`chowk_${chowk.id}`).emit("ambulanceAlert", payload);
          // Also log alert in DB
          await supabase.from("ambulance_alerts").insert({
            ambulance_id: ambulanceId, chowk_id: chowk.id,
            alert_level: alertLevel, eta_seconds: Math.round(etaSec),
            driver_name: driverName, destination
          }).catch(() => {});
          alerts.push({ chowkId: chowk.id, chowkName: chowk.name, alertLevel, etaMin: etaMin.toFixed(1) });
        }
      }
    }
    ...
    if (hospital) {
      const etaToHosp = calcETA(hospital.distanceM, parseFloat(speed)||40);
      io.emit("hospital:eta", {
        ambulanceId, hospitalId: hospital.id, hospitalName: hospital.name,
        distanceM: hospital.distanceM, etaSec: etaToHosp, driverName
      });
    }

    res.json({ ok: true, alerts, hospital: hospital ? { name: hospital.name, distanceM: hospital.distanceM } : null });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Get all live ambulances (admin/map)
app.get("/api/ambulance/live", async (req, res) => {
  const now = Date.now();
  // Only return ambulances active in last 30 seconds
  const active = Object.entries(liveStore.ambulances)
    .filter(([, v]) => now - v.ts < 30000)
    .map(([id, v]) => ({ ambulanceId: id, ...v }));
  res.json(active);
});

// Chowks along a route (called when route is set)
app.post("/api/ambulance/route-chowks", async (req, res) => {
  try {
    const { routeCoords } = req.body; // [[lng,lat], ...]
    const chowks = await getChowksOnRoute(routeCoords);
    res.json(chowks);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Nearest hospital
app.post("/api/ambulance/nearest-hospital", async (req, res) => {
  try {
    const { lat, lng } = req.body;
    const hospital = await getNearestHospital(parseFloat(lat), parseFloat(lng));
    res.json(hospital || { error: "No hospitals in database" });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ════════════════════════════════════════════════════════════════════
// TRAFFIC OFFICER REST ENDPOINTS
// ════════════════════════════════════════════════════════════════════

// Officer registration (pending approval)
app.post("/api/officer/register", async (req, res) => {
  try {
    const { name, email, phone, badge_number } = req.body;
    if (!name || !email) return res.status(400).json({ error: "Name and email required" });
    const { data: ex } = await supabase.from("officers").select("id").eq("email", email.toLowerCase()).single();
    if (ex) return res.status(409).json({ error: "Email already registered" });
    const { error } = await supabase.from("officers").insert({
      id: uuidv4(), name, email: email.toLowerCase(), phone: phone||null,
      badge_number: badge_number||null, status: "pending"
    });
    if (error) throw error;
...
// Officer login with email + one-time code
app.post("/api/officer/login", async (req, res) => {
  try {
    const { email, login_code } = req.body;
    if (!email || !login_code) return res.status(400).json({ error: "Email and login code required" });
    const { data: officer, error } = await supabase.from("officers").select("*").eq("email", email.toLowerCase()).single();
    if (error || !officer) return res.status(404).json({ error: "Officer not found" });
    if (officer.status !== "approved") return res.status(403).json({ error: "Account not approved yet. Contact admin." });
    if (!officer.login_code || officer.login_code !== login_code.trim())
      return res.status(401).json({ error: "Invalid login code" });
    if (officer.code_expires_at && new Date(officer.code_expires_at) < new Date())
      return res.status(401).json({ error: "Login code expired. Contact admin for a new one." });
    // Invalidate code (one-time use)
    await supabase.from("officers").update({ login_code: null, code_expires_at: null, last_login: new Date().toISOString() }).eq("id", officer.id);
    const token = jwt.sign({ id: officer.id, email: officer.email, name: officer.name, role: "officer" }, JWT_SECRET, { expiresIn: "12h" });
    res.json({ token, officer: { id: officer.id, name: officer.name, email: officer.email, badge_number: officer.badge_number } });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
const officerAuth = (req, res, next) => {
  ...
  try {
    ...
    if (d.role !== "officer") return res.status(403).json({ error: "Officer access only" });
    req.officer = d; next();
  } catch { res.status(401).json({ error: "Invalid token" }); }
};

// Get live officers (for map)
app.get("/api/officer/live", async (req, res) => {
  const now = Date.now();
  const active = Object.entries(liveStore.officers)
    .filter(([, v]) => now - v.ts < 30000)
...
// Get all officers
app.get("/api/admin/officers", adminAuth, async (req, res) => {
  const { data, error } = await supabase.from("officers").select("id,name,email,phone,badge_number,status,created_at,last_login").order("created_at", { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

// Approve officer + generate login code
app.post("/api/admin/officers/:id/approve", adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { data: officer } = await supabase.from("officers").select("*").eq("id", id).single();
    if (!officer) return res.status(404).json({ error: "Officer not found" });
    // Generate 8-char alphanumeric code
    const code = Math.random().toString(36).substring(2, 6).toUpperCase() + Math.random().toString(36).substring(2, 6).toUpperCase();
    const expires = new Date(Date.now() + 24*60*60*1000).toISOString(); // 24h
    await supabase.from("officers").update({ status: "approved", login_code: code, code_expires_at: expires }).eq("id", id);
    // Email code to officer
    const emailBody = emailHtml(
      "#0B6E6E,#11B5B5", "✅", "Officer Account Approved!", officer.name,
      `Congratulations! Your traffic officer account for <strong>MediCare+ Smart Ambulance System</strong> has been approved.<br><br>
      <div style="background:#f0fafa;border:1px solid #0B6E6E;border-radius:10px;padding:18px;text-align:center;margin:16px 0">
        <div style="font-size:12px;color:#666;margin-bottom:8px">YOUR ONE-TIME LOGIN CODE</div>
        <div style="font-size:32px;font-weight:900;letter-spacing:6px;color:#0B6E6E;font-family:monospace">${code}</div>
        <div style="font-size:12px;color:#999;margin-top:8px">Expires in 24 hours · One-time use only</div>
      </div>
      Login at the Officer Portal and use this code along with your registered email.<br><br>
      <strong>⚠️ Do not share this code with anyone.</strong>`
    );
    await sendEmail(officer.email, "✅ Officer Account Approved — Your Login Code", emailBody, null, "officer_approval");
    res.json({ message: `Officer approved. Code sent to ${officer.email}`, code_preview: code.substring(0,2)+"******" });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Reject officer
app.post("/api/admin/officers/:id/reject", adminAuth, async (req, res) => {
  const { error } = await supabase.from("officers").update({ status: "rejected" }).eq("id", req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ message: "Officer rejected" });
});

// Resend new login code
app.post("/api/admin/officers/:id/resend-code", adminAuth, async (req, res) => {
  try {
    ...
    await supabase.from("officers").update({ status: "approved", login_code: code, code_expires_at: expires }).eq("id", req.params.id);
    const emailBody = emailHtml("#0B6E6E,#11B5B5","🔑","New Login Code","Officer",
      `A new login code has been generated for your officer account.<br><br>
      <div style="background:#f0fafa;border:1px solid #0B6E6E;border-radius:10px;padding:18px;text-align:center;margin:16px 0">
        <div style="font-size:28px;font-weight:900;letter-spacing:6px;color:#0B6E6E;font-family:monospace">${code}</div>
        <div style="font-size:12px;color:#999;margin-top:8px">Expires in 24 hours · One-time use only</div>
      </div>`);
    await sendEmail(officer.email, "🔑 New Officer Login Code — MediCare+", emailBody, null, "officer_code");
    res.json({ message: "New code sent to officer email" });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// CHOWK CRUD
app.get("/api/admin/chowks", adminAuth, async (req, res) => {
  const { data, error } = await supabase.from("chowks").select("*").order("name");
  if (error) return res.status(500).json({ error: error.message }); res.json(data);
});
app.post("/api/admin/chowks", adminAuth, async (req, res) => {
  const { name, lat, lng, area } = req.body;
  if (!name || !lat || !lng) return res.status(400).json({ error: "name, lat, lng required" });
  const { data, error } = await supabase.from("chowks").insert({ id: uuidv4(), name, lat: parseFloat(lat), lng: parseFloat(lng), area: area||null, is_active: true }).select().single();
  if (error) return res.status(500).json({ error: error.message }); res.json({ message: "Chowk added", chowk: data });
});
app.put("/api/admin/chowks/:id", adminAuth, async (req, res) => {
  const { error } = await supabase.from("chowks").update({ ...req.body }).eq("id", req.params.id);
  if (error) return res.status(500).json({ error: error.message }); res.json({ message: "Updated" });
});
app.delete("/api/admin/chowks/:id", adminAuth, async (req, res) => {
  const { error } = await supabase.from("chowks").update({ is_active: false }).eq("id", req.params.id);
  if (error) return res.status(500).json({ error: error.message }); res.json({ message: "Deactivated" });
});
...
app.post("/api/admin/hospitals", adminAuth, async (req, res) => {
  const { name, lat, lng, address, phone, beds } = req.body;
  if (!name || !lat || !lng) return res.status(400).json({ error: "name, lat, lng required" });
  const { data, error } = await supabase.from("hospitals").insert({ id: uuidv4(), name, lat: parseFloat(lat), lng: parseFloat(lng), address: address||null, phone: phone||null, beds: beds?parseInt(beds):null, is_active: true }).select().single();
  if (error) return res.status(500).json({ error: error.message }); res.json({ message: "Hospital added", hospital: data });
});
app.put("/api/admin/hospitals/:id", adminAuth, async (req, res) => {
  const { error } = await supabase.from("hospitals").update({ ...req.body }).eq("id", req.params.id);
  if (error) return res.status(500).json({ error: error.message }); res.json({ message: "Updated" });
});
app.delete("/api/admin/hospitals/:id", adminAuth, async (req, res) => {
  const { error } = await supabase.from("hospitals").update({ is_active: false }).eq("id", req.params.id);
  if (error) return res.status(500).json({ error: error.message }); res.json({ message: "Deactivated" });
});

// Public lists (for driver/officer apps)
app.get("/api/chowks",    async (req, res) => { const { data } = await supabase.from("chowks").select("*").eq("is_active",true).order("name"); res.json(data||[]); });
app.get("/api/hospitals", async (req, res) => { const { data } = await supabase.from("hospitals").select("id,name,lat,lng,address,phone").eq("is_active",true).order("name"); res.json(data||[]); });
...
io.on("connection", (socket) => {
  ...
  // ── Driver joins and broadcasts location ──────────────────────
  socket.on("driver:join", ({ ambulanceId, driverName }) => {
    socket.ambulanceId = ambulanceId;
    socket.join(`amb_${ambulanceId}`);
    console.log(`🚑 Driver joined: ${ambulanceId} (${driverName})`);
    io.emit("ambulance:online", { ambulanceId, driverName, ts: Date.now() });
  });

  socket.on("driver:location", async (data) => {
    const { ambulanceId, lat, lng, speed } = data;
    if (!ambulanceId || !lat || !lng) return;
    const stored = liveStore.ambulances[ambulanceId];
    if (stored) {
      liveStore.ambulances[ambulanceId] = { ...stored, lat, lng, speed: speed||0, ts: Date.now() };
    }
    io.emit("ambulance:location", { ...data, ts: Date.now() });
  });

  // ── Officer connects, gets auto-assigned to nearest chowk ─────
  socket.on("officer:join", async ({ officerId, name, lat, lng, token: oToken }) => {
    // Verify token
    try { jwt.verify(oToken, JWT_SECRET); } catch { socket.emit("officer:error", "Invalid token"); return; }
    socket.officerId = officerId;
    // Find nearest chowk
    const nearest = await getNearestChowk(lat, lng);
    let chowkId = null, chowkName = null;
    if (nearest && nearest.distanceM < 500) { // within 500 m
      chowkId = nearest.id; chowkName = nearest.name;
      socket.join(`chowk_${chowkId}`);
      console.log(`👮 Officer ${name} → chowk ${chowkName}`);
    }
    liveStore.officers[officerId] = { lat, lng, name, chowkId, chowkName, ts: Date.now() };
    socket.emit("officer:assigned", { chowkId, chowkName, distanceM: nearest?.distanceM });
    io.emit("officer:location", { officerId, lat, lng, name, chowkId, chowkName, ts: Date.now() });
  });

  socket.on("officer:location", async (data) => {
    const { officerId, lat, lng, speed } = data;
    if (!officerId) return;
    const stored = liveStore.officers[officerId];
    if (!stored) return;
    const newDist = haversine(lat, lng, stored.lat||lat, stored.lng||lng);
    // Re-assign if moved > 100m from last known position
    if (newDist > 100) {
      const nearest = await getNearestChowk(lat, lng);
      if (nearest && nearest.distanceM < 500 && nearest.id !== stored.chowkId) {
        if (stored.chowkId) socket.leave(`chowk_${stored.chowkId}`);
        socket.join(`chowk_${nearest.id}`);
        stored.chowkId = nearest.id; stored.chowkName = nearest.name;
        socket.emit("officer:assigned", { chowkId: nearest.id, chowkName: nearest.name });
      }
    }
    liveStore.officers[officerId] = { ...stored, lat, lng, ts: Date.now() };
    io.emit("officer:location", { officerId, lat, lng, name: stored.name, chowkId: stored.chowkId, chowkName: stored.chowkName, ts: Date.now() });
  });

  socket.on("disconnect", () => {
    if (socket.ambulanceId) {
      delete liveStore.ambulances[socket.ambulanceId];
      io.emit("ambulance:offline", { ambulanceId: socket.ambulanceId });
    }
    if (socket.officerId) {
      delete liveStore.officers[socket.officerId];
      io.emit("officer:offline", { officerId: socket.officerId });
    }
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(`🚑 Ambulance Tracking: Socket.IO active`);
